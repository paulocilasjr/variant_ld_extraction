...
WINDOW_BP = 500000
PAGE_SIZE = 500
OUTPUT_CSV = Path("outputs/gwas_catalog/risk_allele_tag_ld_matches.csv")
...
def resolve_ld_file() -> Path:
    for candidate in (
        Path("data/inputs/ld_r2_equal_higher_0.8.txt"),
        Path("data/inputs/ld_r2_equal_higher_0.8"),
        Path("ld_r2_equal_higher_0.8.txt"),
        Path("ld_r2_equal_higher_0.8"),
    ):
        if candidate.exists():
            return candidate
...
def main() -> int:
    ...
    tecac_file = Path("data/inputs/TECAC_GWAS_index_SNPs_OCT_2025")
